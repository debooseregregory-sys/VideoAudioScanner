...
def make_thumbnail_bytes(path: str) -> bytes | None:
...
class VideoLibraryWindow(QMainWindow):
    ...
    def _build_ui(self):
        ...
        dl.addWidget(self.delete_button)
    ...
    def load_index_for_folder(self):
        ...
        if items:
            ...
            self.refresh_cards()
            # Eerst de lijst tonen. Zware preview/metadata-verwerking
    ...
    def scan(self):
        ...
        try:
            for path in Path(self.folder).rglob("*"):
                ...
                # Metadata wordt pas na het verzamelen van alle bestanden
    ...
    def _start_metadata_queue(self):
    ...
    def _start_next_thumbnails(self):
    ...
    def delete_selected(self):
        ...
        if not path or not os.path.isfile(path):
            QMessageBox.warning(
                self,
                "Video verwijderen",
                "Selecteer eerst een geldige video."
            )
            return
        ...
        answer = QMessageBox.question(
            self,
            "Naar Prullenbak",
            f"Weet je zeker dat je deze video naar de Windows Prullenbak wilt verplaatsen?\n\n{name}",
        ...
        try:
            # Windows Shell: verplaats naar Prullenbak met Undo mogelijk.
            ...
            if result != 0 or op.fAnyOperationsAborted:
                raise OSError(f"Windows kon het bestand niet naar de Prullenbak verplaatsen. Code: {result}")

            # Uit database verwijderen
            self.db.delete_video(path)

            # Uit huidige lijst verwijderen
            self.items = [
            ...
            self.stats.setText(
                f"{len(self.items):,} video's  •  {format_size(total)}"
            )
            self.status.setText(
                f"🗑 Naar Prullenbak: {name}"
            )
            ...
            QMessageBox.critical(
                self,
                "Video verwijderen",
                f"De video kon niet naar de Prullenbak worden verplaatst.\n\n{exc}"
            )
