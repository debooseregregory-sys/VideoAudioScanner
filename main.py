...
class MainWindow(QMainWindow):
    ...
    def _build_ui(self):
        ...
        browse = QPushButton("Bladeren…"); browse.clicked.connect(self.choose_folder); self.scan_button = QPushButton("▶  Scan starten"); self.scan_button.setObjectName("primary"); self.scan_button.clicked.connect(self.start_scan); self.duplicate_button = QPushButton("DUBBELE VIDEO"); self.duplicate_button.setObjectName("secondary"); self.duplicate_button.clicked.connect(self.open_duplicate_finder); self.cancel_button = QPushButton("STOP"); self.cancel_button.setEnabled(False); self.cancel_button.clicked.connect(self.cancel_scan); self.export_button = QPushButton("CSV exporteren"); self.export_button.setEnabled(False); self.export_button.clicked.connect(self.export_csv)
    ...
    def _selected_items(self) -> list[MediaResult]:
        ...
        for proxy_index in self.table.selectionModel().selectedRows():
            source_index = self.proxy.mapToSource(proxy_index); item = self.model.data(source_index, Qt.ItemDataRole.UserRole)
