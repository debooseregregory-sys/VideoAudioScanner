...
class ThumbnailWorker:
    ...
    def _make_grid(self, source, target):
        ...
        # One FFmpeg process: extract frames at fixed timestamps, scale them into
        # identical cells, then tile the frames into one contact sheet.
        count = max(1, self.screenshot_count)
        rows = (count + self.columns - 1) // self.columns
        interval = max(1, self.interval_minutes * 60)
        start = max(0, self.start_minute * 60)
        select = "".join(f"[{i}:v]" for i in range(count))
        filters = []
        for i in range(count):
            filters.append(
                f"[{i}:v]scale=480:270:force_original_aspect_ratio=decrease,pad=480:270:(ow-iw)/2:(oh-ih)/2:color=black,setsar=1[v{i}]"
            )
        filters.append(f"{select}tile={self.columns}x{rows}:padding=2:margin=2[outv]")
        cmd = [ffmpeg, "-hide_banner", "-loglevel", "error", "-ss", str(start), "-i", source,
               "-filter_complex", ";".join(filters), "-frames:v", "1"]
        # The above filter graph expects multiple streams, so use one input and
        # split it into the requested number of streams, each sampled at its own time.
        split = "".join(f"[s{i}]" for i in range(count))
        graph = [f"[0:v]split={count}{split}"]
        for i in range(count):
            graph.append(f"[s{i}]trim=start={i*interval}:duration=0.04,setpts=PTS-STARTPTS,scale=480:270:force_original_aspect_ratio=decrease,pad=480:270:(ow-iw)/2:(oh-ih)/2:color=black,setsar=1[v{i}]")
        graph.append(f"{select}tile={self.columns}x{rows}:padding=2:margin=2[outv]")
        cmd = [ffmpeg, "-hide_banner", "-loglevel", "error", "-ss", str(start), "-i", source,
               "-filter_complex", ";".join(graph), "-map", "[outv]", "-frames:v", "1"]
...
class ThumbnailToolWindow(QDialog):
    ...
    def _build_ui(self):
        ...
        self.interval = self._spin(2, 1, 1440, " min"); form.addRow("Interval:", self.interval)
    ...
    def make_preview(self):
        if self._busy or not self.selected_files:
            if not self.selected_files: self.status.setText("Kies eerst een video.")
            return
