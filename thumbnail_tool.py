...
class ThumbnailWorker:
    """Create one contact sheet per video.

    The user specifies only how many snapshots are wanted. Their timestamps are
    calculated automatically across the complete video duration. This avoids
    guessing an interval and avoids the fragile multi-input filter graph that was
    ...
    def _ffprobe(self):
        ...
        if not ffmpeg:
            return None
        p = Path(ffmpeg)
        candidate = p.with_name("ffprobe.exe" if os.name == "nt" else "ffprobe")
    ...
    def _make_grid(self, source, target):
        ...
        count = max(1, int(self.screenshot_count))
        columns = max(1, min(int(self.columns), count))
        rows = (count + columns - 1) // columns

        # Spread snapshots evenly over the entire video. With one snapshot use
        # the middle; with multiple snapshots use start through end.
        if count == 1:
        ...
        try:
            for index, timestamp in enumerate(timestamps):
                ...
                # Seek independently for each frame. This is deliberately simple
                # and reliable across MP4/MKV/AVI/etc., rather than using a fragile
                # giant filter graph.
            ...
            # Feed the generated stills to one final FFmpeg tile operation.
            inputs = []
            for frame in frames:
                inputs.extend(["-i", str(frame)])
            labels = "".join(f"[{i}:v]" for i in range(len(frames)))
            graph = f"{labels}tile={columns}x{rows}:padding=3:margin=3[outv]"
            cmd = [
                ffmpeg, "-hide_banner", "-loglevel", "error",
                *inputs,
...
class ThumbnailToolWindow(QDialog):
    ...
    def _build_ui(self):
        ...
        # Only the number of snapshots is user-controlled. Their timing is
        # calculated automatically from the actual video duration.
        ...
        self.columns.setToolTip("Aantal kolommen in het samengestelde raster.")
        ...
        self.width.setToolTip("0 = automatische rasterbreedte. De afzonderlijke beelden zijn 480×270.")
    ...
    def _apply_theme(self):
        self.setStyleSheet("""
        QDialog{background:#101216;color:#fff}
        QLabel{color:#fff}
        QLabel#title{color:#fff;font-size:30px;font-weight:900}
        QLabel#subtitle{color:#fff;font-size:14px}
        QLabel#count{color:#fff;background:#182235;border:1px solid #315f9e;border-radius:7px;padding:8px 12px}
        ...
        QLabel#fileList{color:#fff;background:#12151a;border:1px solid #282d35;border-radius:7px;padding:10px}
        QLabel#preview{color:#fff;background:#0c0f13;border:1px dashed #3b424d;border-radius:7px}
        QLabel#previewName{color:#fff}
        QLabel#status{color:#fff}
        ...
        QComboBox QAbstractItemView{background:#1e2127;color:#fff;selection-background-color:#315f9e;selection-color:#fff}
        QPushButton{background:#292e36;border:1px solid #3c424c;border-radius:7px;padding:9px 14px;color:#fff;font-weight:600}
        ...
        QProgressBar{background:#171a20;border:1px solid #303640;border-radius:6px;text-align:center;color:#fff;min-height:18px}
        QProgressBar::chunk{background:#315f9e;border-radius:5px}
        QToolTip{background:#1e2127;color:#fff;border:1px solid #4a515d;padding:5px}
        """)
    ...
    def choose_folder(self):
        ...
        try:
            paths = [
                str(path) for path in sorted(Path(folder).iterdir())
    ...
    def _tasks(self):
        ...
        return [
            (source, str(Path(folder) / (Path(source).stem + f"_grid.{ext}")))
    ...
    def make_preview(self):
        ...
        worker = ThumbnailWorker(
            source, target, ext, self.quality.value(), self.width.value(), "preview",
            screenshot_count=self.screenshot_count.value(), columns=self.columns.value(),
        )
        self.status.setText("Video analyseren en rastervoorbeeld maken…")
        self._start(worker)
    ...
    def generate_thumbnails(self):
        ...
        self.status.setText("Video's analyseren en rasters maken…")
        self._start(worker)
        ...
        if mode == "preview":
            if ok:
                ...
                self.preview_name.setText(
                    f"{Path(self.selected_files[0]).name} — {self.screenshot_count.value()} automatisch verdeelde beelden"
                )
