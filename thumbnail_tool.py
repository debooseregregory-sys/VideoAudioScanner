...
class ThumbnailWorker:
    """Maak per video één contact sheet met automatisch verdeelde screenshots."""
    ...
    def _duration(self, source):
        ...
        if not ffprobe:
            return None, "FFmpeg/FFprobe kon niet worden gevonden."
        ...
        try:
            result = subprocess.run(
                ...
                text=True, encoding="utf-8", errors="replace",
                creationflags=self._creationflags(), check=False,
            )
    ...
    def _make_grid(self, source, target):
        ...
        # Verdeel de beelden over de volledige video. Eén beeld = midden van de video.
        if count == 1:
        ...
        try:
            ...
            # NIET 'tile' gebruiken: tile heeft maar één video-input.
            # xstack accepteert meerdere inputs en zet ze in een raster.
            inputs = []
            labels = []
            layout = []
            for i, frame in enumerate(frames):
                ...
                col = i % columns
                row = i // columns
                layout.append(f"{col * 480}_{row * 270}")

            graph = (
                "".join(labels)
                + f"xstack=inputs={len(frames)}:layout='{'|'.join(layout)}':fill=black"
                + "[outv]"
            )
            cmd = [
                ffmpeg, "-hide_banner", "-loglevel", "error",
                *inputs, "-filter_complex", graph, "-map", "[outv]", "-frames:v", "1"
            ]
            if self.width > 0:
            ...
            for frame in frames:
                try: frame.unlink(missing_ok=True)
                except OSError: pass
            try: temp_dir.rmdir()
            except OSError: pass
...
class ThumbnailToolWindow(QDialog):
    ...
    def _build_ui(self):
        ...
        header = QHBoxLayout(); text_box = QVBoxLayout()
        title = QLabel("THUMBNAIL TOOL"); title.setObjectName("title"); text_box.addWidget(title)
        subtitle = QLabel("Maak automatisch één samengesteld raster uit screenshots verspreid over de volledige video.")
        subtitle.setObjectName("subtitle"); subtitle.setWordWrap(True); text_box.addWidget(subtitle)
        header.addLayout(text_box, 1)
        self.count_label = QLabel("0 video's geselecteerd"); self.count_label.setObjectName("count")
        header.addWidget(self.count_label, alignment=Qt.AlignmentFlag.AlignTop); root.addLayout(header)

        content = QHBoxLayout(); content.setSpacing(14); left = QVBoxLayout(); left.setSpacing(12)
        source = QWidget(); source.setObjectName("panel"); sl = QVBoxLayout(source); sl.setContentsMargins(16,14,16,14)
        st = QLabel("1. Video's kiezen"); st.setObjectName("panelTitle"); sl.addWidget(st)
        buttons = QHBoxLayout()
        ...
        self.file_list = QLabel("Nog geen video's geselecteerd."); self.file_list.setObjectName("fileList"); self.file_list.setWordWrap(True); self.file_list.setMinimumHeight(110); sl.addWidget(self.file_list)
        ...
        self.screenshot_count = self._spin(20, 1, 100, " stuks"); self.screenshot_count.setToolTip("Aantal screenshots. De tool verdeelt ze automatisch over de hele video."); form.addRow("Screenshots:", self.screenshot_count)
        ...
        output_row = QHBoxLayout(); self.output = QLineEdit(); self.output.setPlaceholderText("Doelmap voor rasters"); output_row.addWidget(self.output,1)
        browse = QPushButton("Bladeren…"); browse.clicked.connect(self.choose_output); output_row.addWidget(browse); form.addRow("Doelmap:", output_row)
        left.addWidget(settings); left.addStretch(); content.addLayout(left,1)

        preview_panel = QWidget(); preview_panel.setObjectName("panel"); pl = QVBoxLayout(preview_panel); pl.setContentsMargins(16,14,16,14)
        pt = QLabel("2. Voorbeeld raster"); pt.setObjectName("panelTitle"); pl.addWidget(pt)
        ...
        self.preview_button = QPushButton("Voorbeeld maken"); self.preview_button.setObjectName("secondary"); self.preview_button.clicked.connect(self.make_preview); pl.addWidget(self.preview_button)
        content.addWidget(preview_panel,1); root.addLayout(content,1)
    ...
    def _output_folder(self):
        ...
        return folder or (str(Path(self.selected_files[0]).parent/"thumbnails") if self.selected_files else "")
    def _tasks(self):
        folder=self._output_folder(); Path(folder).mkdir(parents=True,exist_ok=True); ext=self.format.currentData()
    ...
    def make_preview(self):
        ...
        ext=self.format.currentData(); target=str(Path(tempfile.gettempdir())/f"thumbnail_preview.{ext}")
        worker=ThumbnailWorker(self.selected_files[0],target,ext,self.quality.value(),self.width.value(),"preview",screenshot_count=self.screenshot_count.value(),columns=self.columns.value())
        self.status.setText("Video analyseren en rastervoorbeeld maken…"); self._start(worker)
    def generate_thumbnails(self):
        ...
        worker=ThumbnailWorker("","",self.format.currentData(),self.quality.value(),self.width.value(),"batch",tasks=tasks,screenshot_count=self.screenshot_count.value(),columns=self.columns.value())
        self.status.setText("Video's analyseren en rasters maken…"); self._start(worker)
    ...
    def _on_result(self,mode,ok,target,error,done,total):
        ...
        if mode=="preview":
            if ok:
                self._preview_pixmap=QPixmap(target); self._update_preview(); self.preview_name.setText(f"{Path(self.selected_files[0]).name} — {self.screenshot_count.value()} automatisch verdeelde beelden"); self.status.setText("Voorbeeld klaar.")
