...
VIDEO_FILTER = (
    "Video's (*.mp4 *.mkv *.avi *.mov *.m4v *.webm *.ts *.mts "
    "*.m2ts *.wmv *.flv *.mpeg *.mpg);;Alle bestanden (*)"
)
...
class ThumbnailWorker:
    ...
    def _run(self, cmd, target=None):
        ...
        if process.returncode != 0:
            return False, (
                err or "FFmpeg gaf een onbekende fout."
            ).strip()
        ...
        if target:
            ...
            if (
                ...
                return False, (
                    "FFmpeg heeft geen geldige afbeelding aangemaakt."
                )
    ...
    def _duration(self, source):
        ...
        if not probe:
            return (
                None,
                "FFprobe kon niet worden gevonden naast FFmpeg.",
            )
        ...
        try:
            ...
            if duration <= 0:
                return (
                    None,
                    "De videolengte kon niet worden bepaald.",
                )
    ...
    def _extract_frame(self, ffmpeg, source, timestamp, path):
        cmd = [
            ...
            "-vf",
            (
                f"scale={self.CELL_W}:{self.CELL_H}:"
                "force_original_aspect_ratio=decrease,"
                f"pad={self.CELL_W}:{self.CELL_H}:"
    ...
    @staticmethod
    def _get_timestamp_font(size=24):
        candidates = []
    ...
    def _add_timestamp(self, image, timestamp):
        ...
        font = self._get_timestamp_font(24)
        ...
        x1 = image.width - margin - box_width
        y1 = image.height - margin - box_height
    ...
    def _make_grid(self, source, target):
        if not PIL_AVAILABLE:
            return (
                False,
                "Pillow ontbreekt. Installeer met: "
        ...
        # Gelijkmatig verspreid over de volledige video.
        timestamps = [
        ...
        temp = Path(
            tempfile.mkdtemp(prefix="vas_thumb_")
        )

        frames = []

        try:
            # Eerst alle screenshots maken.
            for i, timestamp in enumerate(timestamps):
                if self.stop_event.is_set():
                    return False, "Gestopt door gebruiker."

                frame_path = temp / f"frame_{i:03d}.jpg"

                ok, error = self._extract_frame(
                    ffmpeg,
                    source,
                    timestamp,
                    frame_path,
                ...
                if not ok:
                    return (
                        False,
                        f"Screenshot {i + 1}/{count} "
                        f"mislukt: {error}",
                    )

                frames.append(frame_path)

            # Eén grote samengestelde afbeelding.
            sheet = Image.new(
                "RGB",
                (
                    columns * self.CELL_W,
            ...
            # Elk screenshot afzonderlijk voorzien van timestamp.
            for i, frame_path in enumerate(frames):
                with Image.open(frame_path) as image:
                    ...
                    x = (i % columns) * self.CELL_W
                    y = (i // columns) * self.CELL_H
            ...
            # Optionele eindbreedte.
            if (
                self.width > 0
                and sheet.width != self.width
            ):
                new_height = max(
                    1,
                    round(
    ...
    def run(self, signals):
        if self.mode == "preview":
            ...
            signals.finished.emit(
                ...
                self.target,
                error,
                self.screenshot_count if ok else 0,
...
class ThumbnailToolWindow(QDialog):
    ...
    def __init__(self, parent=None):
        ...
        self.setWindowTitle(
            "VideoAudioScanner - Thumbnail Tool"
        )
    ...
    def _build_ui(self):
        ...
        subtitle = QLabel(
            "Maak automatisch één samengesteld raster "
            "met screenshots verspreid over de volledige video."
        )
        ...
        self.screenshot_count = self._spin(
            20,
            1,
            100,
            " stuks",
        ...
        self.screenshot_count.setToolTip(
            "Aantal screenshots; automatisch gelijkmatig "
            "verdeeld over de volledige video."
        )
        ...
        self.show_timestamp.setToolTip(
            "Toont HH:MM:SS rechtsonder op elk screenshot."
        )
        ...
        self.preview_name.setObjectName(
            "previewName"
        )
    ...
    def _format_changed(self):
        self.quality.setEnabled(
            self.format.currentData() == "jpg"
        )
    ...
    def choose_folder(self):
        ...
        try:
            ...
            QMessageBox.critical(
                self,
                "Thumbnail Tool",
                "De videomap kon niet worden gelezen."
                f"\n\n{exc}",
            )
            return
    ...
    def _set_files(self, paths):
        ...
        self.count_label.setText(
            f"{len(self.selected_files)} video's geselecteerd"
        )
        ...
        return [
            (
                ...
                str(
                    ...
                    / (
                        Path(source).stem
                        + f"_grid_{count}shots_"
                        f"{actual_columns}x{rows}."
                        f"{extension}"
                    )
                ),
            )
    ...
    def make_preview(self):
        ...
        worker = ThumbnailWorker(
            ...
            self.width.value(),
            "preview",
            screenshot_count=self.screenshot_count.value(),
            columns=self.columns.value(),
            show_timestamp=self.show_timestamp.isChecked(),
        )

        self.status.setText(
            "Rastervoorbeeld wordt gemaakt…"
        )
    ...
    def generate_thumbnails(self):
        ...
        try:
            ...
            QMessageBox.critical(
                self,
                "Thumbnail Tool",
                "Doelmap kon niet worden aangemaakt."
                f"\n\n{exc}",
            )
            return
        ...
        if mode == "preview":
            ...
            if ok:
                ...
                timestamp_text = (
                    ...
                    else " — tijdstempels uit"
                )

                self.preview_name.setText(
                    f"{Path(self.selected_files[0]).name}"
                    f" — {count} beelden — "
                    f"{columns} × {rows} raster"
                    f"{timestamp_text}"
                )
    ...
    def reject(self):
        if self._busy:
            QMessageBox.information(
                self,
                "Thumbnail Tool",
                "Stop de huidige bewerking eerst.",
            )
            return
