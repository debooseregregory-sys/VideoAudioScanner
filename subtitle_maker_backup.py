...
class SubtitleMakerWindow(QMainWindow):
    ...
    def process_finished(self, exit_code, exit_status):
        ...
        # Het SRT-bestand is het echte resultaat.
        # Als het bestaat en niet leeg is, is de opdracht geslaagd.
        if srt.is_file() and srt.stat().st_size > 0:
            self.status_label.setText("Ondertitels succesvol gemaakt.")

            self.log.append("")
            self.log.append("=" * 60)
            self.log.append("KLAAR")
            self.log.append("=" * 60)
            self.log.append(f"SRT-bestand:`n{srt}")

            QMessageBox.information(
                self,
                "Subtitle Maker",
                (
                    "De ondertitels zijn succesvol gemaakt.`n`n"
                    f"{srt}"
                ),
            )
            return
        ...
        self.log.append("")
        self.log.append("=" * 60)
        self.log.append(f"FOUT - exit code {exit_code}")
        self.log.append("=" * 60)
        ...
        QMessageBox.critical(
            ...
            "Subtitle Maker",
            (
                "Faster-Whisper XXL kon de ondertitels niet maken.`n`n"
                f"Exit code: {exit_code}`n`n"
